...
func NewAsyncLogWriter(filePath string, bufSize int64) *AsyncLogWriter {
...
func (w *AsyncLogWriter) initLogFile() error {
	var (
		fd  *os.File
		err error
	)
...
func (w *AsyncLogWriter) Start() error {
...
func (w *AsyncLogWriter) flushBuffer() {
	for {
		select {
		case msg := <-w.buf:
			w.SyncWrite(msg)
...
func (w *AsyncLogWriter) SyncWrite(msg []byte) {
	w.rotateFile()
	if w.fd != nil {
		w.fd.Write(msg)
	}
...
func (w *AsyncLogWriter) rotateFile() {
	select {
	case <-w.hourTicker.C:
...
func (w *AsyncLogWriter) Write(msg []byte) (n int, err error) {
...
func (w *AsyncLogWriter) flushAndClose() error {
	if w.fd == nil {
		return nil
	}
...
func (w *AsyncLogWriter) timeFilePath(filePath string) string {
