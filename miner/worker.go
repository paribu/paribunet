...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case head := <-w.chainHeadCh:
			...
			timestamp = time.Now().Unix()
			if p, ok := w.engine.(*bouleuterion.Bouleuterion); ok {
				signedRecent, err := p.SealRecently(w.chain, head.Block.Header())
				if err != nil {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Ethash != nil) || (w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.Bouleuterion != nil && w.chainConfig.Bouleuterion.Period > 0)) {
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case ev := <-w.chainSideCh:
			if _, ok := w.engine.(*bouleuterion.Bouleuterion); ok {
				continue
			}
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	if w.current.gasPool == nil {
		...
		w.current.gasPool.SubGas(params.SysTxsGas)
	}
	...
	if delay != nil {
		stopTimer = time.NewTimer(*delay - w.config.DelayBroadcast)
		log.Debug("Time left for mining work", "left", (*delay - w.config.DelayBroadcast).String(), "leftover", w.config.DelayBroadcast)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	syscontracts.ForkForSystemContracts(w.chainConfig, header.Number, env.state)
