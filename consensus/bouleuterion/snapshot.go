...
type Snapshot struct {
	config   *params.BouleuterionConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(
	config *params.BouleuterionConfig,
...
func loadSnapshot(config *params.BouleuterionConfig, sigCache *lru.ARCCache, db ethdb.Database, hash common.Hash, ethAPI *ethapi.PublicBlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("bouleuterion-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("bouleuterion-"), s.Hash[:]...), blob)
}
...
func (s *Snapshot) suspectedValidator() common.Address {
	validators := s.validators()
